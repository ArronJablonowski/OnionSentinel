"""Pure DHCP and Software Inventory contracts for the incident broker."""
...
def _validate_software_record(
    ...
    *,
    ...
    if not isinstance(record, dict) or frozenset(record) not in SOFTWARE_RECORD_KEY_SETS:
    ...
    if set(record) == SOFTWARE_OS_RECORD_KEYS:
        ...
        os_present = bool(
            record["operating_system_type"] or record["operating_system_version"]
        )
        if source == "osquery_apps":
            if os_present and (
                record["operating_system_source"] != "osquery_manager.result:host.os"
