...
def post_alerts_to_webhook(
    ...
    if alert_delivery.delivery_mode(config) == "ssh_batch":
        return _post_ssh_alerts(config, alerts, conn)
...
def _record_ssh_batch_results(
    conn: sqlite3.Connection,
    claimed: dict[str, dict],
    response: dict,
) -> tuple[int, int, int]:
    posted_count = retryable_failures = permanent_failures = 0
    results = {
    ...
    for delivery_id, message in claimed.items():
        ...
        if result.get("ok"):
            ...
            # A poison message is retained for inspection but cannot hold every
            # newer LAN alert behind it forever.
    ...
    return posted_count, retryable_failures, permanent_failures


def _deliver_claimed_ssh_batch(
    config: dict,
    conn: sqlite3.Connection,
    claimed: dict[str, dict],
) -> dict:
    try:
        return alert_delivery.deliver_ssh_batch(config, list(claimed.values()))
...
def _drain_ssh_alert_outbox(
    ...
    for batch in alert_delivery.split_batches(config, messages):
        ...
        if not claimed:
            continue
        response = _deliver_claimed_ssh_batch(config, conn, claimed)
        counts = _record_ssh_batch_results(conn, claimed, response)
        posted_count += counts[0]
        retryable_failures += counts[1]
        permanent_failures += counts[2]
    if retryable_failures or permanent_failures:
        raise RuntimeError(
            "SSH alert intake completed with "
            f"{retryable_failures} retryable and "
            f"{permanent_failures} permanent rejection(s)"
        )
    return posted_count


def _drain_webhook_alert_outbox(
    config: dict,
    conn: sqlite3.Connection,
    pending: list[dict],
) -> int:
...
def drain_alert_outbox(config: dict, conn: sqlite3.Connection) -> int:
    ...
    limit = max(
        1,
        min(int(webhook.get("outbox_drain_limit", 1000) or 1000), 10000),
    )
    pending = alert_outbox.pending(conn, limit)
    if alert_delivery.delivery_mode(config) == "ssh_batch":
        return _drain_ssh_alert_outbox(config, conn, pending)
    return _drain_webhook_alert_outbox(config, conn, pending)
...
def _parse_relay_args() -> tuple[argparse.ArgumentParser, argparse.Namespace]:
...
def _run_pcap_cycle(
    config: dict,
    pruned_runtime_files: int,
    relay_root_storage: dict,
) -> int:
...
def _apply_webhook_override(config: dict, args: argparse.Namespace) -> None:
    ...
    # Prefer env over argv for the systemd path so tokens do not leak through
    # process listings. The argv option remains for isolated tests.
    config["webhook"]["token"] = (
        args.webhook_token
        if args.webhook_token is not None
        else os.environ.get("RELAY_WEBHOOK_TOKEN", "")
    )


def _pull_batch_context(config: dict) -> dict:
...
def _post_quiet_relay_heartbeat(
    config: dict,
    context: dict,
) -> bool:
    if context["new_alerts"]:
        return False
    heartbeat = build_relay_heartbeat(
        context["batch"],
        alert_count=len(context["all_alerts"]),
        dropped_count=context["dropped_count"],
        filtered_count=len(context["filtered_alerts"]),
        new_count=len(context["new_alerts"]),
        duplicate_count=context["duplicate_count"],
        posted_count=context["posted_count"],
        first_rule=context["first_rule"],
    )
    return post_relay_heartbeat(config, heartbeat)


def _process_pull_batch(config: dict, context: dict) -> None:
    with closing(connect_db(config)) as conn:
        # Important order: filter -> dedupe -> durable queue -> post -> mark seen.
        new_alerts, duplicate_count = filter_unseen_alerts(
            conn, context["filtered_alerts"]
        )
        ...
        all_alerts = context["all_alerts"]
        context.update({
            "new_alerts": new_alerts,
            "duplicate_count": duplicate_count,
            "saved_alert_paths": saved_alert_paths,
            "delivery_enabled": delivery_enabled,
            "queued_count": queued_count,
            "posted_count": posted_count,
            "outbox_counts": outbox_counts,
            "first_rule": (
                all_alerts[0].get("rule_name") if all_alerts else "none"
            ),
        })
        context["heartbeat_posted"] = _post_quiet_relay_heartbeat(
            config, context
        )
...
def _pull_cycle_result(
    ...
    return {
        "saved": str(context["output_path"]),
        "source": context["batch"].get("source"),
        "exported_at": context["batch"].get("exported_at"),
        "alert_count": len(context["all_alerts"]),
        "dropped_alert_count": context["dropped_count"],
        "filtered_alert_count": len(context["filtered_alerts"]),
        "new_alert_count": len(context["new_alerts"]),
        "duplicate_alert_count": context["duplicate_count"],
        "saved_new_alert_files": len(context["saved_alert_paths"]),
        "posted_webhook_alerts": context["posted_count"],
        "queued_webhook_alerts": context["queued_count"],
        "outbox_pending_alerts": context["outbox_counts"].get("pending", 0),
        "outbox_dead_letter_alerts": context["outbox_counts"].get(
            "dead_letter", 0
        ),
        "alert_delivery_mode": alert_delivery.delivery_mode(config),
        "posted_webhook_heartbeat": context["heartbeat_posted"],
        "first_rule": context["first_rule"],
...
def _run_pull_cycle(
    ...
    context = _pull_batch_context(config)
    _process_pull_batch(config, context)
    result = _pull_cycle_result(
        config, context, pruned_runtime_files, relay_root_storage
    )
    print(json.dumps(result, sort_keys=True))
    return 0
...
def main() -> int:
    ...
    if args.process_pcap_requests:
        return _run_pcap_cycle(
            config, pruned_runtime_files, relay_root_storage
        )
    if not args.pull_once:
        parser.error("Choose --pull-once or --process-pcap-requests")
    _apply_webhook_override(config, args)
    return _run_pull_cycle(config, pruned_runtime_files, relay_root_storage)
