...
this SQLite outbox before any webhook call so a Mac Studio outage cannot make
...
def pending(conn: sqlite3.Connection, limit: int = 1000) -> list[dict]:
    ...
    for alert_id, payload_json, attempt_count in rows:
        try:
            ...
            mark_failure(conn, alert_id, "stored outbox payload is not valid JSON")
            continue
...
def counts(conn: sqlite3.Connection) -> dict[str, int]:
    ...
    result = {"pending": 0, "delivering": 0, "delivered": 0}
