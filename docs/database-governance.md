...
| --- | --- | --- | --- |
| Mac alert-store SQLite | Alerts, analyst state, jobs, notification/enrichment, incidents, PCAP, analysis | Hourly verified SQLite backup and daily recovery bundle | Onion Sentinel startup schema modules; aggregate version not yet persisted |
...
The initial catalog records eight concrete gaps:

- alert-store and Relay SQLite do not persist aggregate schema versions;
- their startup DDL is not one atomic migration;
