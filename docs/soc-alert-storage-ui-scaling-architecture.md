...
The Onion Sentinel UI uses SQLite as its operational source and enriches detail
views from Markdown files in:
...
Security Onion
  ...
  -> dedicated Onion Sentinel API/UI backed by SQLite
```
...
## Onion Sentinel Backend Recommendation
...
The dedicated Onion Sentinel service serves those fragments through:
...
Fragment publication is atomic. The builder renders each group into a temporary
file in the live details directory, then replaces that group's prior fragment
with `os.replace()`. Stale fragments are removed only after all current groups
have been published. The builder must never clear the live details directory
before rebuilding it because the API remains online and concurrent analysts
must continue receiving either the prior complete fragment or the new complete
fragment, never a transient 404 or partial file.
...
http://10.77.7.225:8766/soc-alerts-status.json
```
...
`data-layout-valid`. Both the builder and dedicated service validate section count and
...
The dedicated service serves a Server-Sent Events stream at:
...
- Treats enabled Ollama models and each independently enabled model in the fixed
  Codex CLI catalog (`gpt-5.5`, `gpt-5.6-sol`, `gpt-5.6-terra`, and
  `gpt-5.6-luna`) as the approved global roster. Each Codex row also selects
  one reasoning effort. Each Cyber
...
node for accepted alerts. Onion Sentinel should link to Markdown when a report
...
The Onion Sentinel API prefers this summary table for:
