...
- Analyst-entered workflow state is persisted in SQLite in the `analyst_alert_status` table. This keeps user decisions such as `acknowledged`, `suppressed`, repeat count at decision time, typed reason, and update timestamp separate from raw alert evidence while still keeping the complete state in the same backed-up alert store.
