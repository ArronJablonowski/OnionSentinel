...
def _load_contradiction_policy():
    path = Path(__file__).resolve().with_name("adjudication_contradiction_policy.py")
    spec = importlib.util.spec_from_file_location(
        "onion_sentinel_adjudication_contradiction_policy", path
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load adjudication contradiction policy")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


_CONTRADICTION_POLICY = _load_contradiction_policy()
...
def adjudication_verdict_contradictions(
    ...
    return _CONTRADICTION_POLICY.verdict_contradictions(
