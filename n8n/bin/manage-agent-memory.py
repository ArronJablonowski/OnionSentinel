...
def _add_quarantine_parser(subparsers: Any) -> None:
    ...
    quarantine.add_argument(
        "--scope", choices=("role", "shared", "both"), default="shared"
    )
    quarantine.add_argument(
        "--apply",
        action="store_true",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    _add_quarantine_parser(subparsers)
    return parser


def _writeback(
    args: argparse.Namespace, role_file: Path, shared_file: Path,
) -> dict[str, Any]:
    ...
    return persist_memory_candidates(
...
def _quarantine(
    ...
    results = [
        quarantine_bpfdoor_code_zero_memory(
            path, apply=args.apply, record_ids=args.record_id,
        )
        for path in paths
    ]
    return {
...
def dispatch(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.command == "prepare":
        return build_agent_execution_context(
            agent_role=args.agent,
    ...
    if args.command == "writeback":
        return _writeback(args, role_file, shared_file)
    return _quarantine(args, role_file, shared_file)
