    ...
    return f'''
      <section class="llm-current-card" aria-label="Current alert being analyzed">
        ...
        <div class="llm-current-meta">
          ...
          <span><b>Started</b><em id="llm-current-started">{html.escape(started or 'n/a')}</em></span>
          <span><b>Alerts</b><em id="llm-current-count">{html.escape(str(alert.get('alert_count') or 0))}</em></span>
          <span><b>Queue</b><em id="llm-current-queue">{html.escape(str(queue_size))}</em></span>
        </div>
      </section>'''
...
def reports_page_section(_reports: list[AlertReport]) -> str:
    ...
    if not rows:
        rows = '<tr><td colspan="10" class="llm-empty-row">No local LLM analysis logs found yet.</td></tr>'
    return f'''
...
(() => {
  ...
  const rowHtml = log => {
    ...
    const gpu = log.gpu_temperature_celsius_max != null ? `${Number(log.gpu_temperature_celsius_max).toFixed(1)} C` : 'Unavailable';
  ...
  async function loadLogs(reset=false) {
    ...
    try {
      ...
      if (body) body.innerHTML = (data.logs || []).length ? data.logs.map(rowHtml).join('') : '<tr><td colspan="10" class="llm-empty-row">No local LLM analysis logs found yet.</td></tr>';
...
.metrics .system-health-metric-main span{{display:block!important;margin:0!important;min-width:0!important;color:#aeb9c7!important;font-size:12px!important;line-height:1.15!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}}
.metrics .system-health-metric-main b{{color:#8ff4ff!important}}
...
.ai-status-analyzing{{color:var(--cyan)!important;animation:ai-status-analyzing-pulse 1.25s ease-in-out infinite;text-shadow:0 0 8px rgba(34,211,238,.20)}}@keyframes ai-status-analyzing-pulse{{0%,100%{{color:#0e7490;text-shadow:0 0 4px rgba(34,211,238,.10);filter:brightness(.76)}}50%{{color:#8ff4ff;text-shadow:0 0 12px rgba(143,244,255,.54),0 0 26px rgba(34,211,238,.28);filter:brightness(1.18)}}}}
...
function restoreExpandedApiGroup(expandedId){{if(!expandedId)return;const group=document.querySelector(`tbody.report-row-group[data-report-id="${{CSS.escape(expandedId)}}"]`);if(!group||getComputedStyle(group).display==='none')return;selectedGroup=group;group.classList.add('expanded');const row=group.querySelector('.report-row');row?.classList.add('selected');row?.setAttribute('aria-selected','true');loadGroupDetail(group);syncPinnedContent(group);updateDetailViewport();updatePinnedRow()}}function renderApiAlerts(data,reset=true){{const table=document.querySelector('.alert-table'),mobileList=document.querySelector('.mobile-alert-list');if(!table||!data||!Array.isArray(data.alerts))return;const expandedId=selectedGroup?.classList.contains('expanded')?selectedGroup.dataset.reportId:null;selectedGroup=null;const rows=data.alerts.map(apiRowHtml).join('');table.querySelectorAll('tbody.report-row-group').forEach(row=>row.remove());table.insertAdjacentHTML('beforeend',rows);if(mobileList){{mobileList.innerHTML='';mobileList.insertAdjacentHTML('beforeend',data.alerts.map(apiMobileCardHtml).join(''))}}apiAlertCursor=data.next_cursor||null;renderApiPagination(data);refreshDynamicCollections();bindGroupInteractions();hydrateTriageStatuses();renderLocalLastSeen();updateDetailViewport();applyFilter();restoreExpandedApiGroup(expandedId);pollSocAlertStatus()}}
