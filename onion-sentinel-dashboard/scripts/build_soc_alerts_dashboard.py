...
def normalize_iso_display_text(value: object) -> str:
    ...
    return ISO_DATE_TIME_SEPARATOR_RE.sub(r'\1  ', str(value))
...
def alert_seen_timeline_html(row: sqlite3.Row | dict) -> str:
    ...
    return f'''
<details class="alert-timeline-section" aria-label="Duplicate alert timeline" open>
  ...
  <div class="alert-timeline-body">
    <p class="alert-timeline-summary">Alert firing window <strong>{html.escape(str(normalized[0]['timestamp']))}</strong> to <strong>{html.escape(str(normalized[-1]['timestamp']))}</strong>; over {duration_minutes} minute(s).</p>
...
def report_from_sqlite_row(row: sqlite3.Row | dict, markdown_by_alert_id: dict[str, tuple[Path, str, os.stat_result]], ai_analysis_by_alert_id: dict[str, dict], ai_prompts_by_alert_id: dict[str, dict], running_ai_alert_ids: set[str]) -> AlertReport:
...
def load_reports() -> list[AlertReport]:
    ...
    reports = [report_from_sqlite_row(row, markdown_by_alert_id, ai_analysis_by_alert_id, ai_prompts_by_alert_id, running_ai_alert_ids) for row in aggregated_rows]
    return sorted(reports, key=lambda r: (r.criticality_rank, r.mtime, r.title.lower()), reverse=True)
...
def human_time(ts: float) -> str:
    return dt.datetime.fromtimestamp(ts).astimezone().replace(microsecond=0).isoformat().replace('T', '  ')


def iso_utc_time(ts: float) -> str:
    """Render alert timestamps consistently for SOC table comparisons."""
    value = dt.datetime.fromtimestamp(ts, tz=dt.timezone.utc)
    timespec = 'milliseconds' if value.microsecond else 'seconds'
    return value.isoformat(timespec=timespec).replace('T', '  ').replace('+00:00', 'Z')
...
def last_seen_iso_for(report: AlertReport) -> str:
    return iso_utc_time(last_seen_ts_for(report))
...
def ai_activity_state(reports: list[AlertReport]) -> dict[str, object]:
    ...
    return {
        'active': active,
        'label': 'AI:Analyzing' if active else 'AI:Idle',
        'detail': f'{counts["analyzing"]} active / {counts["queued"]} queued' if active else f'{counts["queued"]} queued',
        'counts': counts,
    }
...
def write_status_json(reports: list[AlertReport]) -> Path:
    ...
    payload = {
        'generated_at': dt.datetime.now(dt.timezone.utc).replace(microsecond=0).isoformat().replace('T', '  ').replace('+00:00', 'Z'),
...
def write_n8n_beacon_json(reports: list[AlertReport]) -> Path:
    ...
    payload = {
        'generated_at': iso_utc_time(latest_report.alert_ts) if latest_report else dt.datetime.now(dt.timezone.utc).replace(microsecond=0).isoformat().replace('T', '  ').replace('+00:00', 'Z'),
        'stage': 'seeded',
        'ok': True,
...
def build_html(reports: list[AlertReport]) -> str:
    ...
    severity_labels = {'critical': 'Critical', 'high': 'High', 'medium': 'Medium', 'low': 'Low', 'informational': 'Info'}
    ...
    total_severity_html = ''.join(f'<span class="sev-chip sev-{level}"><b>{total_severity_counts[level]}</b> {severity_labels[level]}</span>' for level in severity_levels)
    latest_extra_html = (
    ...
    avg_size_text = human_size(round(total_bytes / len(reports))) if reports else '0 B'
    largest = max(reports, key=lambda r: r.size) if reports else None
    size_extra_html = (
        f'<span class="metric-detail-row"><b>Average</b><span>{avg_size_text}</span></span>'
        f'<span class="metric-detail-row"><b>Largest</b><span>{human_size(largest.size)}</span></span>'
    ) if largest else '<span class="metric-detail-row"><b>Average</b><span>0 B</span></span>'
    ai_state = ai_activity_state(reports)
    ai_activity_active = bool(ai_state['active'])
    ai_activity_class = ' ai-activity-active' if ai_activity_active else ''
    ai_activity_label = str(ai_state['label'])
    ai_activity_detail = str(ai_state['detail'])
    ai_activity_extra_html = render_ai_activity_extra(ai_state)
    ai_activity_html = (
        f'<div id="ai-activity-card" class="metric-card ai-activity-card{ai_activity_class}" aria-live="polite">'
        f'<span class="metric-icon ai-activity-icon" aria-hidden="true"><img src="assets/metric-ai.png" alt=""></span>'
        f'<div class="metric-main"><strong id="ai-activity-label">{ai_activity_label}</strong><span id="ai-activity-detail">{ai_activity_detail}</span></div>'
        f'<div id="ai-activity-extra" class="metric-extra metric-detail">{ai_activity_extra_html}</div></div>'
    )
    ...
    mobile_cards = '\n'.join(
        ...
        <article class="mobile-alert-card" data-mobile-report-id="{html.escape(r.digest)}" data-acknowledged="false" data-suppressed="false" data-rule-id="{html.escape(r.rule_id, quote=True)}" data-rule-name="{html.escape(r.rule_name, quote=True)}" data-suppressed-next-minutes="{repeat_next_minutes_by_digest.get(r.digest, '')}">
          <div class="mobile-card-top"><span class="severity-label severity-text-{html.escape(criticality_class(r.criticality))}">{html.escape(r.criticality)}</span><span class="mobile-card-time">Last Seen <span data-last-seen-utc="{html.escape(last_seen_iso_for(r), quote=True)}">{html.escape(last_seen_iso_for(r))}</span></span></div>
          <h2>{html.escape(r.title)}</h2>
          <p>{html.escape(ai_summary_for(r))}</p>
          <div class="mobile-card-meta"><span>Log Source <b>{html.escape(r.alert_source)}</b></span></div>
          <div class="mobile-endpoints"><span><b>Src</b><code>{html.escape(r.source_ip)}:{html.escape(r.source_port)}</code></span><span><b>Dst</b><code>{html.escape(r.destination_ip)}:{html.escape(r.destination_port)}</code></span></div>
          <div class="mobile-card-meta"><span>Count <b>{r.repeat_count}</b></span><span>Risk <b>{risk_score_for(r)}</b></span><span>{ai_status_pill(r)}</span><span>{human_size(r.size)}</span></div>
          <div class="mobile-card-actions"><button class="ack-button" type="button" data-acknowledge="{html.escape(r.digest)}">Acknowledge</button><button class="ack-button suppress-button" type="button" data-suppress="{html.escape(r.digest)}">Suppress</button><details class="mobile-detail"><summary>Drill down</summary><div class="suppression-note" hidden><h3>Suppression Note</h3><p class="suppression-note-text"></p><small class="suppression-note-meta"></small></div><div class="markdown-body">{r.rendered_html}</div></details></div>
        </article>'''
        for r in reports
    )
...
.suppression-network-context{{display:flex;align-items:center;gap:8px;margin:-2px 0 10px;padding:0 2px;color:#c8d5e4;font:12px/1.35 ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"Liberation Mono",monospace;overflow-wrap:anywhere}}
...
let apiAlertCursor=null,apiAlertLoading=false,socApiTableEnabled=true,apiAlertReloadTimer=null,socEventsSource=null,socEventsSignature='',socEventsReloadTimer=null,apiCurrentPage=1,apiTotalPages=1,apiTotalMatching=0;
...
function defaultSortDirection(key){{return ['alert','source_ip','destination_ip','log_source','ai'].includes(key)?'asc':'desc'}}
...
function apiRowHtml(alert){{const id=alert.group_id,level=apiSeverityLevel(alert),label=apiSeverityLabel(alert),title=`[${{label.toUpperCase()}}] ${{alert.rule_name||'Security Onion Alert'}}`,lastSeen=alert.last_seen||'',count=Number(alert.seen_count||0),risk=apiRiskScore(alert),src=alert.source_ip||'n/a',dst=alert.destination_ip||'n/a',port=alert.destination_port||'-',source=alert.event_dataset||'n/a',sizeBytes=Number(alert.payload_size_bytes||0)||0,sizeLabel=formatApiBytes(sizeBytes),body=[label,title,source,src,dst,port,alert.group_key,count,sizeLabel].join(' ').toLowerCase(),status=alert.analyst_status||'open',epoch=parseApiEpoch(lastSeen);return `<tbody class="report-row-group" data-report-id="${{escapeHtml(id)}}" data-title="${{escapeHtml(title.toLowerCase())}}" data-source="${{escapeHtml(source.toLowerCase())}}" data-body="${{escapeHtml(body)}}" data-alert-group-key="${{escapeHtml(alert.group_key||'')}}" data-repeat-count="${{count}}" data-criticality="${{escapeHtml(level)}}" data-ai-status="${{escapeHtml(alert.ai_status_key||'not-queued')}}" data-risk-score="${{risk}}" data-mtime="${{epoch}}" data-alert-ts="${{epoch}}" data-rule-id="" data-rule-name="${{escapeHtml(alert.rule_name||'')}}" data-alert-source="${{escapeHtml(source)}}" data-summary="" data-modified="${{escapeHtml(lastSeen)}}" data-size="${{escapeHtml(sizeLabel)}}" data-size-bytes="${{sizeBytes}}" data-source-ip="${{escapeHtml(src)}}" data-destination-ip="${{escapeHtml(dst)}}" data-destination-port="${{escapeHtml(port)}}" data-source-label="SQLite API" data-acknowledged="${{status==='acknowledged'}}" data-suppressed="${{status==='suppressed'}}"><tr class="report-row" tabindex="0" aria-selected="false"><td class="select-cell"><span class="row-check">✓</span></td><td class="endpoint-cell count-cell"><span class="alert-repeat-count">${{count}}</span></td><td class="severity-cell"><span class="severity-label severity-text-${{escapeHtml(level)}}">${{escapeHtml(label)}}</span></td><td class="last-seen-cell" data-last-seen-utc="${{escapeHtml(lastSeen)}}">${{escapeHtml(lastSeen)}}</td><td class="alert-cell"><strong>${{escapeHtml(title)}}</strong></td><td class="endpoint-cell ip-cell"><code>${{escapeHtml(src)}}</code></td><td class="endpoint-cell ip-cell"><code>${{escapeHtml(dst)}}</code></td><td class="endpoint-cell port-cell"><code>${{escapeHtml(port)}}</code></td><td class="ai-status-cell">${{apiAiPill(alert)}}</td><td class="source-cell"><code>${{escapeHtml(source)}}</code></td><td>${{escapeHtml(sizeLabel)}}</td><td class="wide-only">${{risk}}</td><td class="action-cell"><button class="ack-button" type="button" data-acknowledge="${{escapeHtml(id)}}">Acknowledge</button><button class="ack-button suppress-button" type="button" data-suppress="${{escapeHtml(id)}}">Suppress</button></td><td class="menu-cell">⋮</td></tr><tr class="detail-template-row"><td colspan="14"><div class="detail-template"><div class="detail-label">Detailed Alert Report</div><div class="suppression-note" hidden><h3>Suppression Note</h3><p class="suppression-note-text"></p><small class="suppression-note-meta"></small></div><div class="api-detail-content" data-detail-loaded="false">${{apiDetailHtml(alert)}}</div></div></td></tr></tbody>`}}
function apiMobileCardHtml(alert){{const id=alert.group_id,level=apiSeverityLevel(alert),label=apiSeverityLabel(alert),title=`[${{label.toUpperCase()}}] ${{alert.rule_name||'Security Onion Alert'}}`;return `<article class="mobile-alert-card" data-mobile-report-id="${{escapeHtml(id)}}" data-acknowledged="${{alert.analyst_status==='acknowledged'}}" data-suppressed="${{alert.analyst_status==='suppressed'}}" data-rule-id="" data-rule-name="${{escapeHtml(alert.rule_name||'')}}"><div class="mobile-card-top"><span class="severity-label severity-text-${{escapeHtml(level)}}">${{escapeHtml(label)}}</span><span class="mobile-card-time">Last Seen <span data-last-seen-utc="${{escapeHtml(alert.last_seen||'')}}">${{escapeHtml(alert.last_seen||'')}}</span></span></div><h2>${{escapeHtml(title)}}</h2><p>Grouped API alert. Count ${{Number(alert.seen_count||0)}}.</p><div class="mobile-card-meta"><span>Log Source <b>${{escapeHtml(alert.event_dataset||'n/a')}}</b></span></div><div class="mobile-endpoints"><span><b>Src</b><code>${{escapeHtml(alert.source_ip||'n/a')}}:${{escapeHtml(alert.source_port||'-')}}</code></span><span><b>Dst</b><code>${{escapeHtml(alert.destination_ip||'n/a')}}:${{escapeHtml(alert.destination_port||'-')}}</code></span></div><div class="mobile-card-meta"><span>Count <b>${{Number(alert.seen_count||0)}}</b></span><span>Risk <b>${{apiRiskScore(alert)}}</b></span><span>${{apiAiPill(alert)}}</span><span>API</span></div><div class="mobile-card-actions"><button class="ack-button" type="button" data-acknowledge="${{escapeHtml(id)}}">Acknowledge</button><button class="ack-button suppress-button" type="button" data-suppress="${{escapeHtml(id)}}">Suppress</button><details class="mobile-detail"><summary>Drill down</summary><div class="suppression-note" hidden><h3>Suppression Note</h3><p class="suppression-note-text"></p><small class="suppression-note-meta"></small></div><div class="api-detail-content" data-detail-loaded="false">${{apiDetailHtml(alert)}}</div></details></div></article>`}}
function renderApiPagination(data){{apiCurrentPage=Number(data.page||apiCurrentPage)||1;apiTotalPages=Math.max(1,Number(data.total_pages||1)||1);apiTotalMatching=Number(data.total_matching||0)||0;if(navVisibleCount&&appShell?.dataset.view==='alerts'&&analystStatusParam()==='open'){{navVisibleCount.textContent=String(apiTotalMatching);updateNavAlertSeverity(highestSeverityForGroups([...document.querySelectorAll('tbody.report-row-group')].filter(group=>statusForGroup(group).status==='open')))}}if(data.sort)apiSortKey=String(data.sort);if(data.direction)apiSortDirection=String(data.direction)==='asc'?'asc':'desc';updateSortHeaders();if(apiPageSelect){{apiPageSelect.innerHTML='';for(let page=1;page<=apiTotalPages;page+=1){{apiPageSelect.add(new Option(`Page ${{page}} of ${{apiTotalPages}}`,String(page),false,page===apiCurrentPage))}}apiPageSelect.disabled=apiTotalPages<=1}}if(apiPrevPage)apiPrevPage.disabled=apiCurrentPage<=1;if(apiNextPage)apiNextPage.disabled=apiCurrentPage>=apiTotalPages;if(apiPageStatus){{const start=apiTotalMatching?((apiCurrentPage-1)*apiPageSizeValue())+1:0,end=Math.min(apiCurrentPage*apiPageSizeValue(),apiTotalMatching);apiPageStatus.textContent=`Showing ${{start}}-${{end}} of ${{apiTotalMatching}} grouped detections`}}}}
    ...
    <section id="overview-view" class="view-section overview-view active flow-page-view" aria-label="Autonomous SIEM alert enrichment data flow">
      <section class="flow-product-hero" aria-labelledby="flow-title">
        ...
        <div class="flow-product-map" role="img" aria-label="Animated Security Onion alert pipeline">
          ...
          <div class="flow-ai-branch"><span>prompt + evidence</span></div>
          <article class="flow-system-node node-ollama">
            ...
            <em>Local LLM analysis</em>
          </article>
          <div class="flow-report-branch"><span>AI findings</span></div>
          <article class="flow-system-node node-ai-reports">
            ...
            <div>
              <strong>AI Reports</strong><span>Ollama analysis output</span>
          ...
          <div class="flow-sqlite-branch"><span>alert records</span></div>
          <article class="flow-system-node node-sqlite">
            <span class="flow-logo-ring"><img src="assets/brand/sqlite.svg" alt="SQLite logo"></span>
            <div>
              <strong>SQLite</strong><span>Fast alert-store backend</span>
            ...
            <em>Fast dashboard store</em>
          </article>
          <div class="flow-telegram-branch"><span>high signal notify</span></div>
          <article class="flow-system-node node-telegram">
            <span class="flow-logo-ring"><img src="assets/brand/telegram.svg" alt="Telegram logo"></span>
            <div>
              <strong>Telegram</strong><span>High-signal mobile alerts</span>
            ...
            <em>Mobile notification</em>
          </article>
          <div class="flow-ai-dashboard-stem"><span>AI reports</span></div>
          <div class="flow-sqlite-dashboard-stem"><span>SQLite records</span></div>
          <article class="flow-system-node node-onion-sentinel">
        ...
        <summary>
          <span class="settings-summary-main">
            ...
            <span class="settings-summary-copy">
              ...
              <span class="settings-memory-line">Memory: <code>{analyst_memory_path}</code></span>
            </span>
          </span>
          <code>{prompt_path}</code>
        </summary>
        <div class="settings-panel-top">
          <div>
        ...
        <summary>
          <span class="settings-summary-main">
            ...
            <span class="settings-summary-copy">
              ...
              <span class="settings-memory-line">Memory: <code>{incident_memory_path}</code></span>
            </span>
          </span>
        ...
        <summary>
          <span class="settings-summary-main">
            ...
            <span class="settings-summary-copy">
              ...
              <span class="settings-memory-line">Memory: <code>{engineer_memory_path}</code></span>
            </span>
          </span>
        ...
        <summary>
          <span class="settings-summary-main">
            ...
            <span class="settings-summary-copy">
              ...
              <span class="settings-memory-line">Memory: <code>{hunter_memory_path}</code></span>
            </span>
          </span>
...
.flow-product-hero{position:relative;display:grid;grid-template-columns:minmax(280px,.36fr) minmax(780px,1fr);gap:22px;align-items:stretch;border:1px solid rgba(148,163,184,.14);border-radius:16px;padding:24px;background:linear-gradient(135deg,#0c151f 0%,#101923 58%,#071018 100%);box-shadow:0 22px 48px rgba(0,0,0,.24),inset 0 1px 0 rgba(255,255,255,.035)}
.flow-privacy-toggle{position:absolute;right:18px;top:18px;z-index:10;width:46px;height:46px;display:grid;place-items:center;border:1px solid rgba(34,211,238,.32);border-radius:15px;padding:0;background:rgba(7,16,24,.82);box-shadow:0 14px 30px rgba(0,0,0,.28),0 0 18px rgba(34,211,238,.10),inset 0 1px 0 rgba(255,255,255,.04);cursor:pointer;transition:transform .16s ease,border-color .16s ease,box-shadow .16s ease,filter .16s ease}
...
.flow-product-copy{display:flex;flex-direction:column;justify-content:center;min-width:0;padding:8px 2px}
.flow-product-copy h2{max-width:18ch;margin:8px 0 10px;color:#f5f9ff;font-size:24px;line-height:1.08;letter-spacing:-.025em}
.flow-product-copy p{max-width:48ch;margin:18px 0 0;color:#aab8ca;font-size:14px;line-height:1.62}
...
.flow-product-map{position:relative;display:grid;grid-template-columns:minmax(150px,1fr) minmax(104px,.58fr) minmax(150px,1fr) minmax(104px,.58fr) minmax(150px,1fr);grid-template-rows:158px 74px 158px 74px 176px 82px 158px;gap:8px 14px;align-items:center;min-height:1008px;overflow:hidden;border:1px solid rgba(34,211,238,.13);border-radius:14px;padding:22px;background:radial-gradient(circle at 78% 24%,rgba(34,211,238,.08),transparent 34%),linear-gradient(180deg,rgba(7,16,24,.62),rgba(6,12,20,.90))}
.flow-product-map:before{content:"";position:absolute;inset:18px;border:1px solid rgba(148,163,184,.06);border-radius:12px;pointer-events:none}
.flow-system-node{position:relative;z-index:3;display:grid;grid-template-rows:auto 1fr auto;gap:10px;min-width:0;min-height:158px;height:158px;border:1px solid rgba(148,163,184,.15);border-radius:14px;padding:16px;background:rgba(10,18,27,.92);box-shadow:0 16px 38px rgba(0,0,0,.24),inset 0 1px 0 rgba(255,255,255,.035);transition:transform .16s ease,border-color .16s ease,box-shadow .16s ease}
...
.flow-logo-ring{width:52px;height:52px;display:grid;place-items:center;border:1px solid rgba(148,163,184,.16);border-radius:15px;background:rgba(255,255,255,.035);box-shadow:inset 0 0 20px rgba(34,211,238,.035)}
.flow-logo-ring img{width:34px;height:34px;object-fit:contain;display:block;filter:drop-shadow(0 0 8px rgba(34,211,238,.16))}
.flow-logo-pair{display:flex;align-items:center;gap:8px;margin:0}
.flow-logo-pair .flow-logo-ring{margin:0!important}
.flow-logo-pair img{filter:drop-shadow(0 0 8px rgba(143,244,255,.20))}
.flow-logo-ring span{color:#8ff4ff;font-size:14px;font-weight:950;letter-spacing:0}
.node-security-onion .flow-logo-ring img{width:38px;height:38px}
.node-raspberry-pi .flow-logo-ring img{width:32px;height:32px}
.node-docker .flow-logo-ring img{width:38px;height:38px}
.node-n8n .flow-logo-ring img{width:38px;height:38px}
.node-mac .flow-logo-ring img{width:30px;height:30px}
.node-ollama .flow-logo-ring img{width:28px;height:40px;filter:invert(1) drop-shadow(0 0 8px rgba(34,211,238,.22))}
.node-ai-reports .flow-logo-ring img{width:34px;height:34px}
.node-ai-reports .flow-logo-ring img[alt="JSON logo"]{filter:brightness(1.35) contrast(.92) drop-shadow(0 0 8px rgba(255,255,255,.20)) drop-shadow(0 0 10px rgba(143,244,255,.16))}
.node-sqlite .flow-logo-ring img{width:38px;height:38px}
.node-telegram .flow-logo-ring img{width:36px;height:36px}
.node-onion-sentinel .flow-logo-ring{width:64px;height:64px;border-radius:18px}
.node-onion-sentinel .flow-logo-ring img{width:58px;height:58px}
.flow-system-node strong{display:block;color:#f4f8ff;font-size:15px;line-height:1.2}
.flow-system-node span:not(.flow-logo-ring){display:block;margin-top:6px;color:#91a4ba;font-size:12px}
...
.flow-system-node em{align-self:end;color:#8ff4ff;font-size:10.5px;font-style:normal;font-weight:900;text-transform:uppercase;letter-spacing:.08em}
.flow-node-metrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;align-self:center;min-width:0}
.flow-node-metrics span{display:grid!important;gap:3px;margin:0!important;min-width:0;border:1px solid rgba(34,211,238,.14);border-radius:10px;padding:7px 8px;background:rgba(34,211,238,.045);box-shadow:inset 0 1px 0 rgba(255,255,255,.025)}
.flow-node-metrics b{color:#f5f9ff;font-size:14px;line-height:1;font-weight:950;font-variant-numeric:tabular-nums}
.flow-node-metrics em{align-self:auto;color:#91a4ba;font-size:9px;line-height:1.1;font-weight:850;letter-spacing:.05em;text-transform:uppercase;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.flow-format-metrics{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin-top:10px;min-width:0}
.flow-format-metrics span{display:grid!important;gap:2px;margin:0!important;border:1px solid rgba(143,244,255,.14);border-radius:10px;padding:6px 7px;background:rgba(143,244,255,.045);box-shadow:inset 0 1px 0 rgba(255,255,255,.025)}
.flow-format-metrics b{color:#f5f9ff;font-size:13px;line-height:1;font-weight:950;font-variant-numeric:tabular-nums}
.flow-format-metrics em{align-self:auto;color:#91a4ba;font-size:8.5px;line-height:1.1;font-style:normal;font-weight:850;letter-spacing:.05em;text-transform:uppercase;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.node-ai-reports,.node-sqlite,.node-telegram{height:176px;min-height:176px;gap:6px;padding:12px 14px;grid-template-rows:auto 1fr auto}
.node-ai-reports .flow-format-metrics,.node-sqlite .flow-format-metrics,.node-telegram .flow-format-metrics{margin-top:6px;gap:6px}
.node-ai-reports .flow-format-metrics span,.node-sqlite .flow-format-metrics span,.node-telegram .flow-format-metrics span{padding:4px 6px}
.node-ai-reports .flow-format-metrics b,.node-sqlite .flow-format-metrics b,.node-telegram .flow-format-metrics b{font-size:12.5px}
.node-ai-reports .flow-format-metrics em,.node-sqlite .flow-format-metrics em,.node-telegram .flow-format-metrics em{font-size:8px}
.node-ai-reports>em,.node-sqlite>em,.node-telegram>em{align-self:end;line-height:1.15;max-width:100%;overflow:hidden;text-overflow:ellipsis}
.node-security-onion{grid-column:1;grid-row:1;border-color:rgba(251,113,133,.26)}
.node-raspberry-pi{grid-column:3;grid-row:1;border-color:rgba(197,26,74,.25)}
.node-docker{grid-column:5;grid-row:1;border-color:rgba(36,150,237,.28)}
.node-n8n{grid-column:5;grid-row:3;border-color:rgba(234,75,113,.28)}
.node-mac{grid-column:3;grid-row:3;border-color:rgba(34,197,94,.26)}
.node-ollama{grid-column:1;grid-row:3;border-color:rgba(143,244,255,.24)}
.node-ai-reports{grid-column:1;grid-row:5;border-color:rgba(143,244,255,.24)}
.node-sqlite{grid-column:3;grid-row:5;border-color:rgba(34,211,238,.26)}
.node-markdown{grid-column:4;grid-row:5;border-color:rgba(246,199,109,.26)}
.node-telegram{grid-column:5;grid-row:5;border-color:rgba(42,171,238,.28)}
.node-onion-sentinel{grid-column:1 / 4;grid-row:7;border-color:rgba(34,211,238,.38);box-shadow:0 16px 42px rgba(0,0,0,.26),0 0 30px rgba(34,211,238,.08),inset 0 1px 0 rgba(255,255,255,.04)}
.node-onion-sentinel{grid-template-columns:auto minmax(150px,.72fr) minmax(260px,1fr);grid-template-rows:1fr auto;align-items:center;column-gap:16px}
.node-onion-sentinel .flow-logo-ring{grid-row:1 / 3}
.node-onion-sentinel .flow-node-metrics{grid-column:3;grid-row:1 / 3}
.flow-connector,.flow-ai-branch,.flow-report-branch,.flow-sqlite-branch,.flow-markdown-branch,.flow-telegram-branch,.flow-ai-dashboard-stem,.flow-sqlite-dashboard-stem{position:relative;z-index:2;align-self:center;justify-self:stretch;min-width:0}
.flow-connector,.flow-ai-branch{height:2px;background:linear-gradient(90deg,rgba(34,211,238,.16),rgba(143,244,255,.82),rgba(34,211,238,.16))}
.flow-connector:before,.flow-ai-branch:before,.flow-report-branch:before,.flow-sqlite-branch:before,.flow-markdown-branch:before,.flow-telegram-branch:before,.flow-ai-dashboard-stem:before,.flow-sqlite-dashboard-stem:before{content:"";position:absolute;width:8px;height:8px;border-radius:999px;background:#8ff4ff;box-shadow:0 0 0 4px rgba(34,211,238,.10),0 0 18px rgba(34,211,238,.75)}
.flow-connector:before,.flow-ai-branch:before{left:0;top:50%;transform:translate(-50%,-50%);animation:flow-dot-horizontal 3.6s linear infinite}
.flow-connector:after,.flow-ai-branch:after{content:"";position:absolute;right:-2px;top:50%;width:9px;height:9px;border-top:2px solid #8ff4ff;border-right:2px solid #8ff4ff;transform:translateY(-50%) rotate(45deg)}
.flow-connector span,.flow-ai-branch span,.flow-report-branch span,.flow-sqlite-branch span,.flow-markdown-branch span,.flow-telegram-branch span,.flow-ai-dashboard-stem span,.flow-sqlite-dashboard-stem span{position:absolute;z-index:4;left:50%;top:-58px;max-width:154px;transform:translateX(-50%);white-space:normal;text-align:center;line-height:1.15;border:1px solid rgba(148,163,184,.16);border-radius:999px;padding:7px 12px;color:#c7d4e4;background:#071018;font-size:10px;font-weight:850;box-shadow:0 0 0 6px rgba(7,16,24,.88),0 0 16px rgba(0,0,0,.34)}
.connector-one{grid-column:2;grid-row:1}
.connector-two{grid-column:4;grid-row:1}
.connector-three{grid-column:5;grid-row:2;justify-self:center;width:2px;height:46px;background:linear-gradient(180deg,rgba(143,244,255,.82),rgba(34,211,238,.14))}
.connector-three:before,.flow-report-branch:before,.flow-sqlite-branch:before,.flow-telegram-branch:before,.flow-ai-dashboard-stem:before,.flow-sqlite-dashboard-stem:before{left:50%;top:0;transform:translate(-50%,-50%);animation:flow-dot-vertical 3.6s linear infinite}
.connector-three:after{right:50%;top:auto;bottom:-2px;transform:translateX(50%) rotate(135deg)}
.connector-three span{left:50%;top:-58px;transform:translateX(-50%);max-width:126px}
.connector-four{grid-column:4;grid-row:3}
.connector-four:before,.flow-ai-branch:before{left:auto;right:0;transform:translate(50%,-50%);animation-name:flow-dot-horizontal-reverse}
.connector-four:after{left:-2px;right:auto;transform:translateY(-50%) rotate(225deg)}
.connector-four span{top:-58px}
.flow-ai-branch{grid-column:2;grid-row:3;background:linear-gradient(270deg,rgba(34,211,238,.16),rgba(143,244,255,.82),rgba(34,211,238,.16))}
.flow-ai-branch:after{left:-2px;right:auto;transform:translateY(-50%) rotate(225deg)}
.flow-ai-branch span{top:-58px;bottom:auto}
.flow-report-branch{grid-column:1;grid-row:4;justify-self:center;width:2px;height:46px;background:linear-gradient(180deg,rgba(143,244,255,.82),rgba(34,211,238,.14))}
.flow-report-branch:after,.flow-sqlite-branch:after,.flow-telegram-branch:after{content:"";position:absolute;left:50%;bottom:-2px;width:9px;height:9px;border-right:2px solid #8ff4ff;border-bottom:2px solid #8ff4ff;transform:translateX(-50%) rotate(45deg)}
.flow-report-branch span{left:50%;top:-58px;transform:translateX(-50%);max-width:116px}
.flow-sqlite-branch{grid-column:3;grid-row:4;justify-self:center;width:2px;height:46px;background:linear-gradient(180deg,rgba(143,244,255,.82),rgba(34,211,238,.14))}
.flow-sqlite-branch span{left:50%;top:-58px;transform:translateX(-50%);max-width:116px}
.flow-markdown-branch{grid-column:4;grid-row:4;align-self:end;height:2px;margin-bottom:28px;background:linear-gradient(90deg,rgba(34,211,238,.12),rgba(143,244,255,.58),rgba(34,211,238,.12))}
.flow-markdown-branch:before{left:0;top:50%;transform:translate(-50%,-50%);animation:flow-dot-horizontal 3.6s linear infinite}
.flow-markdown-branch:after{content:"";position:absolute;right:-2px;top:50%;width:9px;height:9px;border-top:2px solid #8ff4ff;border-right:2px solid #8ff4ff;transform:translateY(-50%) rotate(45deg)}
.flow-markdown-branch span{top:12px}
.flow-telegram-branch{grid-column:5;grid-row:4;justify-self:center;width:2px;height:46px;background:linear-gradient(180deg,rgba(143,244,255,.82),rgba(34,211,238,.14))}
.flow-telegram-branch span{left:50%;right:auto;top:-58px;transform:translateX(-50%);max-width:126px}
.flow-ai-dashboard-stem,.flow-sqlite-dashboard-stem{grid-row:6;justify-self:center;align-self:center;width:2px;height:58px;background:linear-gradient(180deg,rgba(143,244,255,.82),rgba(34,211,238,.14))}
.flow-ai-dashboard-stem{grid-column:1}
.flow-sqlite-dashboard-stem{grid-column:3}
.flow-ai-dashboard-stem:after,.flow-sqlite-dashboard-stem:after{content:"";position:absolute;left:50%;bottom:-2px;width:9px;height:9px;border-right:2px solid rgba(143,244,255,.88);border-bottom:2px solid rgba(143,244,255,.88);transform:translateX(-50%) rotate(45deg)}
.connector-one span,.connector-two span,.connector-four span,.flow-ai-branch span,.flow-markdown-branch span{left:50%;right:auto;top:-60px;bottom:auto;transform:translateX(-50%);max-width:148px;text-align:center;white-space:normal;color:#dce9f8;background:rgba(7,16,24,.96);border-color:rgba(143,244,255,.24);box-shadow:0 0 0 8px rgba(7,16,24,.86),0 0 20px rgba(34,211,238,.10)}
.connector-three span,.flow-report-branch span,.flow-sqlite-branch span,.flow-telegram-branch span,.flow-ai-dashboard-stem span,.flow-sqlite-dashboard-stem span{left:calc(50% + 20px);right:auto;top:50%;bottom:auto;transform:translateY(-50%);max-width:136px;text-align:left;white-space:normal}
.connector-one:before{animation-delay:0s}
.connector-two:before{animation-delay:.25s}
.connector-three:before{animation-delay:.5s}
.connector-four:before{animation-delay:.75s}
.flow-ai-branch:before{animation-delay:1s}
.flow-report-branch:before{animation-delay:1.25s}
.flow-sqlite-branch:before{animation-delay:1.5s}
.flow-markdown-branch:before{animation-delay:1.75s}
.flow-telegram-branch:before{animation-delay:2s}
.flow-ai-dashboard-stem:before{animation-delay:2.25s}
.flow-sqlite-dashboard-stem:before{animation-delay:2.5s}
...
@keyframes flow-dot-horizontal-reverse{0%{right:0;opacity:0;transform:translate(50%,-50%) scale(.72)}10%,86%{opacity:1}100%{right:100%;opacity:0;transform:translate(50%,-50%) scale(1.05)}}
@keyframes flow-dot-vertical{0%{top:0;opacity:0;transform:translate(-50%,-50%) scale(.72)}10%,86%{opacity:1}100%{top:100%;opacity:0;transform:translate(-50%,-50%) scale(1.05)}}
...
.flow-summary-grid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:14px;margin-top:16px}
...
@media(max-width:1500px){.flow-product-hero{grid-template-columns:1fr}.flow-product-copy{padding:0}.flow-product-map{min-height:990px}}
@media(max-width:1180px){
  .flow-product-map{grid-template-columns:1fr;grid-template-rows:auto;min-height:0}
  .node-security-onion,.node-raspberry-pi,.node-docker,.node-n8n,.node-mac,.node-ollama,.node-ai-reports,.node-sqlite,.node-markdown,.node-telegram,.node-onion-sentinel,.connector-one,.connector-two,.connector-three,.connector-four,.flow-ai-branch,.flow-report-branch,.flow-sqlite-branch,.flow-markdown-branch,.flow-telegram-branch,.flow-ai-dashboard-stem,.flow-sqlite-dashboard-stem{grid-column:1;grid-row:auto}
  .flow-system-node{height:auto;min-height:136px}
  .node-onion-sentinel{grid-template-columns:auto 1fr}
  .node-onion-sentinel .flow-node-metrics{grid-column:1 / -1;grid-row:auto;grid-template-columns:repeat(2,minmax(0,1fr))}
  .flow-connector,.flow-ai-branch,.flow-report-branch,.flow-sqlite-branch,.flow-markdown-branch,.flow-telegram-branch,.flow-ai-dashboard-stem,.flow-sqlite-dashboard-stem{width:2px;height:44px;justify-self:center;background:linear-gradient(180deg,rgba(34,211,238,.16),rgba(143,244,255,.82),rgba(34,211,238,.16));margin:0}
  .flow-connector:before,.flow-ai-branch:before,.flow-report-branch:before,.flow-sqlite-branch:before,.flow-markdown-branch:before,.flow-telegram-branch:before,.flow-ai-dashboard-stem:before,.flow-sqlite-dashboard-stem:before{left:50%;right:auto;top:0;animation-name:flow-dot-vertical;transform:translate(-50%,-50%)}
  .flow-connector:after,.flow-ai-branch:after,.flow-report-branch:after,.flow-sqlite-branch:after,.flow-markdown-branch:after,.flow-telegram-branch:after,.flow-ai-dashboard-stem:after,.flow-sqlite-dashboard-stem:after{right:50%;left:auto;top:auto;bottom:-2px;transform:translateX(50%) rotate(135deg)}
  .flow-connector span,.flow-ai-branch span,.flow-report-branch span,.flow-sqlite-branch span,.flow-markdown-branch span,.flow-telegram-branch span,.flow-ai-dashboard-stem span,.flow-sqlite-dashboard-stem span{top:50%;bottom:auto;left:calc(50% + 18px);right:auto;transform:translateY(-50%);max-width:170px;text-align:left}
  .flow-summary-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
}
@media(max-width:640px){.flow-product-hero{padding:16px;border-radius:14px}.flow-product-copy h2{font-size:28px}.flow-product-map{padding:14px}.flow-summary-grid{grid-template-columns:1fr}}
</style>
'''
...
def write_site_pages(reports: list[AlertReport]) -> list[Path]:
    ...
    copy_static_assets()
    written: list[Path] = [write_status_json(reports), write_n8n_beacon_json(reports), *write_detail_fragments(reports)]
