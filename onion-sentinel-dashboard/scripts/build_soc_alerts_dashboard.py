...
def incident_response_page_section() -> str:
      ...
      <div class="ir-table-wrap">
        <table class="ir-table">
          ...
          <thead><tr>
            ...
            <th><button class="ir-sort" type="button" data-ir-sort="status">Status</button></th>
            <th><button class="ir-sort" type="button" data-ir-sort="severity">Severity</button></th>
            ...
            <th><button class="ir-sort" type="button" data-ir-sort="source">Source IP</button></th>
            <th><button class="ir-sort" type="button" data-ir-sort="destination">Destination IP</button></th>
            <th><button class="ir-sort" type="button" data-ir-sort="destination_port">Destination Port</button></th>
    ...
    <style>
      ...
      .ir-table{min-width:1560px}.ir-table col.ir-col-agent{width:198px}.ir-agent{white-space:normal;line-height:1.35}.ir-agent-analysis_failed{color:#ff7088}.ir-agent-review_failed{color:#ffb15c}.ir-agent-refresh_failed{color:#ffcb67}
    ...
    (() => {
      ...
      const rowHtml=item=>{const level=severity(item),agentState=item.agent_display_status||item.agent_status,agentLabel=item.agent_display_label||label(item.agent_status);return `<tr class="ir-case-row" tabindex="0" data-case-id="${esc(item.case_id)}" data-final-review-status="${esc(item.final_review_status||'unreviewed')}"><td><button class="ir-expand" type="button" aria-expanded="false" aria-label="Expand incident case">&#9662;</button></td><td><span class="ir-status ir-status-${esc(item.status)}">${esc(label(item.status))}</span></td><td><span class="ir-status ir-severity-${esc(level)}">${esc(level)}</span></td><td class="ir-escalated">${esc(item.escalated_at||'n/a')}</td><td><strong class="ir-alert-title">${esc(item.rule_name||'Security Onion alert')}</strong><span class="ir-muted">${esc(caseSummary(item))}</span>${reviewBadges(item)}</td><td><code class="ir-code">${esc(item.source_ip||'n/a')}</code></td><td><code class="ir-code">${esc(item.destination_ip||'n/a')}</code></td><td><code class="ir-code">${esc(item.destination_port??'n/a')}</code></td><td>${Number(item.seen_count||0)}</td><td><span class="ir-agent ir-agent-${esc(agentState)}">${esc(agentLabel)}</span>${reviewButton(item)}${reanalysisButton(item)}</td></tr><tr class="ir-detail-row" data-detail-for="${esc(item.case_id)}" hidden><td colspan="10"><div class="ir-detail-shell"><div class="ir-detail-content">Loading case evidence...</div></div></td></tr>`};
