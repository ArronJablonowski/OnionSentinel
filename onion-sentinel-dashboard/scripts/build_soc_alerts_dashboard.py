...
def incident_response_page_section() -> str:
      ...
      <div class="ir-table-wrap">
        <table class="ir-table">
          <colgroup>
            ...
            <col class="ir-col-network"><col class="ir-col-count">
            <col class="ir-col-agent">
          </colgroup>
          <thead><tr>
            ...
            <th><button class="ir-sort" type="button" data-ir-sort="agent">Analysis</button></th>
          </tr></thead>
          <tbody id="ir-table-body"><tr><td colspan="7" class="ir-loading">Loading incident cases...</td></tr></tbody>
        </table>
      </div>
    ...
    <style>
      ...
      .ir-table{min-width:1080px}.ir-table col.ir-col-expand{width:52px}.ir-table col.ir-col-case{width:112px}.ir-table col.ir-col-escalated{width:152px}.ir-table col.ir-col-alert{width:auto}.ir-table col.ir-col-network{width:244px}.ir-table col.ir-col-count{width:68px}.ir-table col.ir-col-agent{width:190px}.ir-table th,.ir-table td{padding:13px 12px}.ir-case-row td{background:#09141d}.ir-case-row:nth-of-type(4n+1) td{background:#0a1620}.ir-case-cell{display:grid;gap:7px;align-content:center}.ir-case-cell .ir-status{width:max-content}.ir-case-cell .ir-severity-label{font-size:.68rem;font-weight:900;letter-spacing:.04em;text-transform:uppercase}.ir-escalated{display:grid;gap:3px;white-space:normal}.ir-escalated-date{color:#d7e3ef;font-weight:780}.ir-escalated-time{color:#8397ab;font-size:.75rem}.ir-network-path{display:grid;grid-template-columns:minmax(0,1fr) 18px minmax(0,1.15fr);gap:6px;align-items:center}.ir-network-endpoint{min-width:0}.ir-network-label{display:block;margin-bottom:3px;color:#73879a;font-size:.62rem;font-weight:900;letter-spacing:.05em;text-transform:uppercase}.ir-network-value{display:block;overflow:hidden;color:#d8e7f8;font:700 12px/1.35 ui-monospace,SFMono-Regular,Menlo,monospace;text-overflow:ellipsis;white-space:nowrap}.ir-network-arrow{color:#35d9ec;text-align:center}.ir-count-value{display:inline-grid;min-width:36px;height:30px;padding:0 8px;place-items:center;border:1px solid #214153;border-radius:999px;color:#dffaff;background:#0b1e29;font-weight:900}.ir-analysis-cell{display:grid;gap:8px;justify-items:start}.ir-agent{white-space:normal;line-height:1.35}.ir-agent-analysis_failed{color:#ff7088}.ir-agent-review_failed{color:#ffb15c}.ir-agent-refresh_failed{color:#ffcb67}
    ...
    (() => {
      ...
      const networkHtml=item=>{const source=item.source_ip||'n/a',destination=item.destination_ip||'n/a',port=item.destination_port;return `<div class="ir-network-path"><span class="ir-network-endpoint"><span class="ir-network-label">Source</span><code class="ir-network-value" title="${esc(source)}">${esc(source)}</code></span><span class="ir-network-arrow" aria-hidden="true">→</span><span class="ir-network-endpoint"><span class="ir-network-label">Destination</span><code class="ir-network-value" title="${esc(destination)}${port?':'+esc(port):''}">${esc(destination)}${port?`:${esc(port)}`:''}</code></span></div>`};
      ...
      async function loadReanalysisProgress(runId=''){
        try{
          ...
          renderReanalysisProgress(payload.latest_run);
      ...
      async function load(){
        ...
        }catch(error){errorBox.textContent=`Incident Response queue unavailable: ${error.message}`;errorBox.hidden=false;body.innerHTML='<tr><td colspan="7" class="ir-loading">Incident cases could not be loaded.</td></tr>';mobile.innerHTML=''}
      }
