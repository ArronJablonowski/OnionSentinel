...
@media(max-width:720px){{.severity-chip{{min-height:36px!important}}.api-page-button{{min-height:36px!important}}}}
...
function restoreExpandedApiGroup(expandedId){{if(!expandedId)return;const group=document.querySelector(`tbody.report-row-group[data-report-id="${{CSS.escape(expandedId)}}"]`);if(!group||getComputedStyle(group).display==='none')return;selectedGroup=group;group.classList.add('expanded');const row=group.querySelector('.report-row');row?.classList.add('selected');row?.setAttribute('aria-selected','true');row?.setAttribute('aria-expanded','true');loadGroupDetail(group);syncPinnedContent(group);updateDetailViewport();updatePinnedRow()}}function renderApiAlerts(data,reset=true,scrollAnchor=null){{const table=document.querySelector('.alert-table'),mobileList=document.querySelector('.mobile-alert-list');if(!table||!data||!Array.isArray(data.alerts))return;const expandedId=selectedGroup?.classList.contains('expanded')?selectedGroup.dataset.reportId:scrollAnchor?.id||null;selectedGroup=null;const rows=data.alerts.map(apiRowHtml).join('');table.querySelectorAll('tbody.report-row-group').forEach(row=>row.remove());table.insertAdjacentHTML('beforeend',rows);if(mobileList){{mobileList.innerHTML='';mobileList.insertAdjacentHTML('beforeend',data.alerts.map(apiMobileCardHtml).join(''))}}apiAlertCursor=data.next_cursor||null;renderApiPagination(data);refreshDynamicCollections();bindGroupInteractions();hydrateTriageStatuses();renderLocalLastSeen();updateDetailViewport();applyFilter();restoreExpandedApiGroup(expandedId);window.__socAlertScrollStabilizer?.restore?.(scrollAnchor);pollSocAlertStatus()}}
...
@media(max-width:720px){.hunt-code-card button{min-height:36px;padding:7px 10px}}
</style>
'''
