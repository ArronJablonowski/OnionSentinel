...
def llm_current_panel(current: dict[str, object]) -> str:
    ...
    model = str(current.get('model') or current.get('mode') or 'n/a')
    queue_size = current.get('queue_size', current_llm_queue_size())
    status_label = 'Analyzing now' if status == 'running' else 'Idle'
    return f'''
      ...
      <section class="llm-log-section" aria-label="AI analysis log">
        ...
        <div class="llm-log-table-wrap">
          <table class="llm-log-table">
            ...
            <thead><tr><th>Started</th><th>Count</th><th>Alert(s)</th><th>Runtime</th><th>GPU °C</th><th>GPU %</th><th>CPU °C</th><th>SOC °C</th><th>Max Memory</th><th>Max Power</th><th>Max CPU</th><th>PCAP Size</th><th>Alert Data</th><th>Model</th><th>Detail</th></tr></thead>
...
(() => {
  ...
  async function loadCurrent() {
    try {
      ...
      if (response.ok) renderCurrent(await response.json());
    } catch (_) {}
  }
...
.review-badge-disputed,.review-freshness-stale,.review-coverage-gaps{border-color:rgba(255,112,136,.55);color:#ff8da1;background:rgba(255,112,136,.08)}
...
.analyst-review-panel.review-status-disputed_pending_human{border-color:rgba(255,112,136,.62);box-shadow:inset 3px 0 0 #ff7088}
...
.analyst-review-empty{margin:0;color:#9caec2}.analyst-adjudication-summary{padding:11px;border:1px solid rgba(105,232,154,.35);border-radius:8px;background:rgba(105,232,154,.055);color:#d8e7f8}.analyst-adjudication-summary p{margin:7px 0}.analyst-adjudication-summary small{color:#9caec2}
...
let apiAlertCursor=null,apiAlertLoading=false,apiAlertRequestVersion=0,socApiTableEnabled=true,apiAlertReloadTimer=null,socEventsSource=null,socEventsSignature='',socEventsReloadTimer=null,escalationApiReloadTimer=null,apiCurrentPage=1,apiTotalPages=1,apiTotalMatching=0,apiActiveTotal=0,apiHighestSeverity='none',apiSeverityCounts=null;
...
function apiReviewBadges(alert){{const finalStatus=String(alert.final_review_status||'unreviewed'),statusClass=finalStatus==='disputed_pending_human'?'disputed':finalStatus==='model_consensus'?'consensus':finalStatus,finalLabel=finalStatus==='disputed_pending_human'?'Disputed':finalStatus==='model_consensus'?'Models agree':finalStatus==='reviewer_advisory'?'Reviewer advisory':finalStatus==='adjudicated'?'Adjudicated':'Unreviewed',freshness=String(alert.freshness_status||'not_analyzed'),coverage=String(alert.coverage_status||'unknown'),confidence=String(alert.effective_confidence||alert.analysis_confidence||'');return `<span class="review-badge-row" aria-label="Analysis review state"><span class="review-badge review-badge-${{escapeHtml(statusClass)}}">${{escapeHtml(finalLabel)}}</span><span class="review-badge review-freshness-${{escapeHtml(freshness)}}">Freshness: ${{escapeHtml(freshness.replaceAll('_',' '))}}</span><span class="review-badge review-coverage-${{escapeHtml(coverage)}}">Coverage: ${{escapeHtml(coverage.replaceAll('_',' '))}}</span>${{confidence?`<span class="review-badge review-badge-confidence">Confidence: ${{escapeHtml(confidence)}}</span>`:''}}</span>`}}
...
async function loadApiAlerts(reset=true){{const table=document.querySelector('.alert-table');if(!table||apiAlertLoading)return;const escalationDelay=pendingEscalationRemovalDelay();if(escalationDelay>0){{clearTimeout(escalationApiReloadTimer);escalationApiReloadTimer=window.setTimeout(()=>loadApiAlerts(reset),escalationDelay+25);return}}const requestVersion=apiAlertRequestVersion,scrollAnchor=window.__socAlertScrollStabilizer?.capture?.()||null;if(reset)apiCurrentPage=1;apiAlertLoading=true;socApiTableEnabled=true;if(apiPageStatus)apiPageStatus.textContent='Loading alerts from SQLite API...';try{{const response=await fetch(apiBuildUrl(),{{cache:'no-store'}});if(!response.ok)throw new Error(`HTTP ${{response.status}}`);const data=await response.json();if(requestVersion!==apiAlertRequestVersion){{const retryDelay=pendingEscalationRemovalDelay();clearTimeout(escalationApiReloadTimer);escalationApiReloadTimer=window.setTimeout(()=>loadApiAlerts(true),retryDelay>0?retryDelay+25:0);return}}renderApiAlerts(data,reset,scrollAnchor)}}catch(error){{socApiTableEnabled=false;if(apiPageStatus)apiPageStatus.textContent=`API table unavailable; using static fallback (${{error.message}})`;window.__socAlertScrollStabilizer?.restore?.(scrollAnchor)}}finally{{apiAlertLoading=false}}}}
...
def incident_response_page_section() -> str:
    ...
    (() => {
      ...
      const reviewBadges=item=>{const finalStatus=String(item.final_review_status||'unreviewed'),statusClass=finalStatus==='disputed_pending_human'?'disputed':finalStatus==='model_consensus'?'consensus':finalStatus,statusLabel=finalStatus==='disputed_pending_human'?'Disputed':finalStatus==='model_consensus'?'Models agree':finalStatus==='reviewer_advisory'?'Reviewer advisory':finalStatus==='adjudicated'?'Adjudicated':'Unreviewed',freshness=String(item.freshness_status||'not_analyzed'),coverage=String(item.coverage_status||'unknown'),confidence=String(item.effective_confidence||item.analysis_confidence||'');return `<span class="review-badge-row"><span class="review-badge review-badge-${esc(statusClass)}">${esc(statusLabel)}</span><span class="review-badge review-freshness-${esc(freshness)}">Freshness: ${esc(label(freshness))}</span><span class="review-badge review-coverage-${esc(coverage)}">Coverage: ${esc(label(coverage))}</span>${confidence?`<span class="review-badge review-badge-confidence">Confidence: ${esc(confidence)}</span>`:''}</span>`};
      const queryPurposes={
      ...
      const rowHtml=item=>{const level=severity(item);return `<tr class="ir-case-row" tabindex="0" data-case-id="${esc(item.case_id)}" data-final-review-status="${esc(item.final_review_status||'unreviewed')}"><td><button class="ir-expand" type="button" aria-expanded="false" aria-label="Expand incident case">&#9662;</button></td><td><span class="ir-status ir-status-${esc(item.status)}">${esc(label(item.status))}</span></td><td><span class="ir-status ir-severity-${esc(level)}">${esc(level)}</span></td><td class="ir-escalated">${esc(item.escalated_at||'n/a')}</td><td><strong class="ir-alert-title">${esc(item.rule_name||'Security Onion alert')}</strong><span class="ir-muted">${esc(caseSummary(item))}</span>${reviewBadges(item)}</td><td><code class="ir-code">${esc(item.source_ip||'n/a')}</code></td><td><code class="ir-code">${esc(item.destination_ip||'n/a')}</code></td><td><code class="ir-code">${esc(item.destination_port??'n/a')}</code></td><td>${Number(item.seen_count||0)}</td><td><span class="ir-agent ir-agent-${esc(item.agent_status)}">${esc(label(item.agent_status))}</span>${reviewButton(item)}</td></tr><tr class="ir-detail-row" data-detail-for="${esc(item.case_id)}" hidden><td colspan="10"><div class="ir-detail-shell"><div class="ir-detail-content">Loading case evidence...</div></div></td></tr>`};
      const mobileHtml=item=>{const level=severity(item);return `<article class="ir-mobile-card" data-mobile-case="${esc(item.case_id)}" data-final-review-status="${esc(item.final_review_status||'unreviewed')}"><button class="ir-mobile-toggle" type="button" aria-expanded="false"><span class="ir-mobile-top"><span class="ir-status ir-severity-${esc(level)}">${esc(level)}</span><span class="ir-agent ir-agent-${esc(item.agent_status)}">${esc(label(item.agent_status))}</span></span><strong class="ir-alert-title">${esc(item.rule_name||'Security Onion alert')}</strong><span class="ir-muted">${esc(caseSummary(item))} | ${esc(item.source_ip||'n/a')} &gt; ${esc(item.destination_ip||'n/a')}${item.destination_port?':'+esc(item.destination_port):''} | ${Number(item.seen_count||0)} alert(s)</span>${reviewBadges(item)}</button><div class="ir-mobile-detail" hidden><div class="ir-mobile-review-action">${reviewButton(item)}</div><div class="ir-detail-content">Loading case evidence...</div></div></article>`};
