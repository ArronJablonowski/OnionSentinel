...
def asset_inventory_page_section() -> str:
    ...
    <section id="asset-inventory-view" class="view-section active asset-view" aria-label="Asset inventory">
      ...
      <div id="dhcp-review-modal" class="dhcp-review-modal" role="dialog" aria-modal="true" aria-labelledby="dhcp-review-title" hidden>
        <form id="dhcp-review-form" class="dhcp-review-card" novalidate>
          ...
          <p id="dhcp-review-auth" class="dhcp-review-auth"><span id="dhcp-review-auth-status">Checking Administration access…</span> <a id="dhcp-review-admin-login" href="/admin/login?resume=asset-review" target="onion-sentinel-admin-auth" rel="noopener">Sign in to Administration</a>. The DHCP observation is revalidated inside the database transaction before any change is committed.</p>
    ...
    <style>
      ...
      .asset-table tr.asset-promoted td{animation:asset-promoted 2.8s ease-out}.dhcp-review-auth.authenticated span{color:#69e89a}.dhcp-review-auth.authenticated a{display:none}@keyframes asset-promoted{0%{background:#123d38}100%{background:transparent}}
    </style>
    <script>
    (()=> {
      ...
      let dhcpItems=new Map(),reviewItem=null,reviewMode='',adminAuthenticated=null,adminPollTimer=null,adminWindow=null,resumeAfterAuth=false,recentlyPromotedAssetId='';
      ...
      function updateAdminStatus(authenticated){adminAuthenticated=authenticated===true;reviewAuth.classList.toggle('authenticated',adminAuthenticated);reviewAuthStatus.textContent=adminAuthenticated?'Administration session active.':'Administration sign-in required.'}
      async function refreshAdminSession(){try{const response=await fetch('/api/admin/session-status',{cache:'no-store',credentials:'same-origin'});const payload=await response.json();updateAdminStatus(response.ok&&payload.authenticated===true);return adminAuthenticated}catch(_){updateAdminStatus(false);return false}}
      ...
      async function commitReview(){if(!reviewItem)return;reviewError.hidden=true;reviewSubmit.disabled=true;const promotion=reviewMode==='promote';reviewSubmit.textContent=promotion?'Promoting…':'Approving…';try{const endpoint=promotion?'/api/assets/promote-dhcp':'/api/assets/approve-dhcp-ip-change';const response=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json','X-Onion-Sentinel-Request':'dashboard'},credentials:'same-origin',body:JSON.stringify(reviewPayload())});const result=await response.json();if(response.status===403){updateAdminStatus(false);resumeAfterAuth=true;startAdminPolling();showReviewError('Administration sign-in is required. Sign in using the link below; this change will resume automatically.');return}if(!response.ok||result.ok===false)throw new Error(result.error||`HTTP ${response.status}`);const promotedAssetId=String(result.asset_id||reviewItem.authoritative_asset?.asset_id||'');recentlyPromotedAssetId=promotedAssetId;closeReview();pageOffset=0;if(promotedAssetId)search.value=promotedAssetId;assetSignature='';dhcpSignature='';await Promise.all([load(),loadDhcp()]);const promotedRow=document.querySelector(`[data-asset-id="${CSS.escape(promotedAssetId)}"]`);promotedRow?.scrollIntoView({behavior:'smooth',block:'center'});status.textContent=`${promotedAssetId} was added to the authoritative Asset Inventory. ${status.textContent}`}catch(error){showReviewError(`Asset review was not committed: ${error.message}`)}finally{reviewSubmit.disabled=false;if(!reviewModal.hidden)reviewSubmit.textContent=reviewMode==='promote'?'Promote asset':'Approve IP change'}}
      async function submitReview(event){event.preventDefault();if(!reviewItem||!validateReview())return;reviewError.hidden=true;if(adminAuthenticated!==true){resumeAfterAuth=true;adminWindow=window.open('/admin/login?resume=asset-review','onion-sentinel-admin-auth','popup,width=520,height=700');startAdminPolling();showReviewError('Administration sign-in is required. Complete sign-in in the opened window; this change will resume automatically.');return}await commitReview()}
      function loadDhcp(){
