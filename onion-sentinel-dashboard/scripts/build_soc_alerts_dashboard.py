...
def build_nav_html(active_page: str, report_count: int) -> str:
    links = []
    for key, filename, title, _subtitle in PAGE_DEFS:
        active_class = ' active' if key == active_page else ''
        count = f'<span class="nav-count" id="soc-alerts-nav-count">{report_count}</span>' if key == 'alerts' else ''
        icon = NAV_ICONS[key]
        links.append(
...
def render_static_page(shell_html: str, page_key: str, reports: list[AlertReport]) -> str:
    ...
    rendered = re.sub(r'<nav class="nav">.*?</nav>', build_nav_html(page_key, len(reports)), rendered, count=1, flags=re.S)
