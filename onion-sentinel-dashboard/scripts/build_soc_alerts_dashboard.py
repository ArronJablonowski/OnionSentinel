...
def software_inventory_page_section() -> str:
    ...
    (()=> {
      ...
      const operatingSystemEvidence=item=>{const os=operatingSystem(item),provenance=os.source?[os.source,os.confidence?`${os.confidence} confidence`:''].filter(Boolean).join(' · '):'Not observed';return `<dt>Operating system type</dt><dd>${esc(os.type||'Not observed')}</dd><dt>Full OS version</dt><dd>${esc(os.version||'Not observed')}</dd><dt>OS evidence</dt><dd>${esc(provenance)}</dd>`};
