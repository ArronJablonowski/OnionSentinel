...
.health-pcap-recent td:nth-child(3)::before{content:"Outcome"}.health-pcap-recent td:nth-child(4)::before{content:"Request"}.health-pcap-recent td:nth-child(5)::before{content:"Group"}.health-pcap-recent td:nth-child(6)::before{content:"Size"}.health-pcap-recent td:nth-child(7)::before{content:"Error"}
  ...
  function renderPcapHealth(pcap) {
    ...
    if (pcapDetails) {
      ...
      const recentHtml = recent.length
        ? `${renderPcapTableControls(recent.length)}<div class="health-pcap-recent"><table><thead><tr><th>Updated</th><th>Status</th><th>Outcome</th><th>Request</th><th>Group</th><th>Size</th><th>Error</th></tr></thead><tbody>${visibleRecent.map(item => `<tr><td><code>${esc(item.updated_at ? fmt(item.updated_at) : 'n/a')}</code></td><td><span class="pcap-status-pill pcap-status-${esc(String(item.status || 'none').toLowerCase())}">${esc(item.status || 'n/a')}</span></td><td>${esc(item.outcome || 'n/a')}</td><td><code>${esc(item.request_id || 'n/a')}</code></td><td><code>${esc(item.group_id || 'n/a')}</code></td><td>${esc(bytes(item.artifact_size_bytes || 0))}</td><td>${esc(item.error || '')}</td></tr>`).join('')}</tbody></table></div>`
