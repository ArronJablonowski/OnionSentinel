...
DHCP asset discovery reuses the existing incident-query key and forced
`export-incident-evidence` command. That wrapper routes the exact DHCP contract
to the fixed helper, which accepts only a bounded UTC window and result size.
The caller cannot
